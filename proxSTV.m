...
% fun_val       Evolution of the objective function during the
% iterations.迭代期间目标函数的变化
...
[maxiter,L,tol,optim,verbose,img,project,P,bc,snorm,order,kernel,showfig]=...
  process_options(varargin,'maxiter',100,'L',8,'tol',1e-5,'optim','fgp',...
...
if isempty(L)
  L=16/1.25;%Lipschitz constant
end

if isempty(P)
  [nx, ny, nc]=size(y);%y是一个三阶张量
  P=zeros([nx ny 2 NGx*NGy*nc]);%返回一个多维零数组
end

count=0;
w=sqrt(kernel);%'kernel'= fspecial('gaussian',[3 3],0.5)，w是一个3*3矩阵
...
if verbose
  ...
  fprintf('*********************************************************\n');
  fprintf('#iter     relative-dif   \t fun_val         Duality Gap        ISNR       PSNR\n')
  fprintf('===============================================================================\n');
end
switch optim
  case 'fgp'
    ...
    F=P;
    for i=1:maxiter
      K=y-lambda*AdjJacobianOp2D(F,w,y,bc);
      Pnew=F+(1/(L*lambda))*JacobianOp2D(project(K),w,y,bc);
      ...
      if verbose
        if ~isempty(img)
          k=y-lambda*AdjJacobianOp2D(P,w,y,bc);
           x=project(k);
          ...
          fprintf('%3d \t %10.5f \t %10.5f \t %2.8f \t %2.8f \t %0.4f\n',i,re,fun_val(i),dual_gap,ISNR(i),PSNR(i));
        else
          k=y-lambda*AdjJacobianOp2D(P,w,y,bc);
          x=project(k);
          fun_val(i)=cost(y,x,w,lambda,bc,snorm,order);
  ...
  case 'gp'
    ...
    for i=1:maxiter
      K=y-lambda*AdjJacobianOp2D(P,w,y,bc);
      Pnew=P+(1/(L*lambda))*JacobianOp2D(project(K),w,y,bc);
      Pnew=projectLB(Pnew,snorm,order);
...
x=project(y-lambda*AdjJacobianOp2D(P,w,y,bc));
